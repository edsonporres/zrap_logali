...
CLASS lhc_Order IMPLEMENTATION.
  ...
  METHOD acceptOrder.
    MODIFY ENTITIES OF zcd_i_r_orden_ap IN LOCAL MODE
    ENTITY Order
    ...
    FAILED failed
    REPORTED reported.
    READ ENTITIES OF zcd_i_r_orden_ap IN LOCAL MODE
 ENTITY Order
 FIELDS ( Country
          Createon
          Deliverydate
  ...
  FAILED failed
  REPORTED reported.
    SELECT MAX( id ) FROM zorden_ap INTO @DATA(lv_id).
    DATA(lv_today) = cl_abap_context_info=>get_system_date( ).
    DATA lt_create TYPE TABLE FOR CREATE zcd_i_r_orden_ap\\Order.
    ...
    MODIFY ENTITIES OF zcd_i_r_orden_ap
  ...
  METHOD validateStatus.
    READ ENTITY zcd_i_r_orden_ap\\Order FIELDS ( Orderstatus ) WITH
    ...
    LOOP AT lt_Order_result INTO DATA(ls_Order_result).
      CASE ls_Order_result-Orderstatus.
        ...
        WHEN OTHERS.
          ...
          %msg = new_message( id = 'Z_MESSAGE_ORDER_AP'
          number = '001'
